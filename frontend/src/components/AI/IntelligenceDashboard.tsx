...
type Tab = 'overview' | 'chat' | 'birth_chart' | 'personality' | 'dosha' | 'compat';
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const reqs: Promise<any>[] = [
        ...
        api.post(`/api/v1/ai/proposals/${proposalId}/personality`).then(res => setPersonalityData(res.data.personality)).catch(() => {}),
        api.post(`/api/v1/ai/proposals/${proposalId}/dosha-report`).then(res => setDoshaData(res.data)).catch(() => {}),
        api.post(`/api/v1/astrology/calculate-chart`, { proposal_id: proposalId }).then(res => setChartData(res.data.chart)).catch(() => {})
      ];
  ...
  return (
    <div className="card animate-in">
      ...
      <div style={{ padding: '0 24px 24px' }}>
        ...
        {activeTab === 'personality' && <PersonalitySection proposalId={proposalId} preloadedData={personalityData} isAnalyzing={analyzing} />}

        {activeTab === 'dosha' && <DoshaReportSection proposalId={proposalId} preloadedData={doshaData} isAnalyzing={analyzing} />}

        {activeTab === 'compat' && compareTarget && (
        ...
        {activeTab === 'chat' && (
          <AIChatSection proposalId={proposalId} />
        )}
